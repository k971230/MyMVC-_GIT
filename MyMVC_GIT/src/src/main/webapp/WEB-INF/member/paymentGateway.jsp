...
$(document).ready(function() {
   ...
   IMP.init('imp58145141');  // 중요!!  아임포트에 가입시 부여받은 "가맹점 식별코드". 
   ...
   IMP.request_pay({
       ...
       buyer_tel : '${requestScope.mobile}',    // 구매자 전화번호 (필수항목) 밑은 생략했음 같은 방식으로 하면된다.
       buyer_addr : '',  
       buyer_postcode : '',
	   ...
	   */
		...
		*/
		//	opener.location.href = "javascript:goCoinUpdate('${requestScope.userid}','${requestScope.coinmoney}')";
			window.opener.goCoin_DB_Update('${requestScope.userid}','${requestScope.coinmoney}');
		//  $(opener.location).attr("href", "javascript:goCoinUpdate('${idx}','${coinmoney}');");
			
		    self.close(); // 자신의 팝업창을 닫은 것 이다.
			
        } else {	// 카드결제가 실패했다면
        	
        	alert("결제에 실패하였습니다.");
            location.href="<%= ctx_Path%>/index.up";
