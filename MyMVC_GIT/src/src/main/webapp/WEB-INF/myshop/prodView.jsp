...
<script type="text/javascript">
   ...
   function goOrder() {
      ...
      if( ${not empty sessionScope.loginuser} ) {
           ...
           var currentcoin = Number("${sessionScope.loginuser.coin}"); // 현재코인액
           var totalSaleprice = Number("${requestScope.pvo.saleprice}") * Number($("input#spinner").val()); // 제품총판매가 
         
           var str_totalSaleprice = totalSaleprice.toLocaleString('en'); // 자바스크립트에서 숫자 3자리마다 콤마 찍어주기  
          var str_currentcoin = currentcoin.toLocaleString('en');       // 자바스크립트에서 숫자 3자리마다 콤마 찍어주기 
           ...
           if( totalSaleprice > currentcoin ) {
            ...
            return;
                  ...
                  var bool = confirm("총주문액 : "+str_totalSaleprice+"원 결제하시겠습니까?");
                 ...
                 if(bool) {
                    ...
                    $.ajax({
                        ...
                        data:{"pnumjoin":"${requestScope.pvo.pnum}",
                             ...
                             "totalPricejoin":Number("${requestScope.pvo.saleprice}")*Number($("input#spinner").val()),
                             "sumtotalPrice":Number("${requestScope.pvo.saleprice}")*Number($("input#spinner").val()),
                             "sumtotalPoint":Number("${requestScope.pvo.point}")*Number($("input#spinner").val())},
                        dataType:"JSON",     
                        success:function(json){
                           if(json.isSuccess == 1) {
      ...
      <div class="col-md-6 pl-5 text-left">
          ...
          <form name="cartOrderFrm">       
             <ul class="list-unstyled mt-3">
               ...
               <li>
                  <button type="button" class="btn btn-secondary btn-sm mr-3" onClick="">장바구니담기</button>
